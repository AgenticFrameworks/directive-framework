...
- [ ] **B5 — Verbatim-required strings grep-verified against the actual artifact.** When
  ...
  the failure surfaces only at build. This covers EVERY string a smoke bar asserts —
  including out-of-scope-guard literals on UNCHANGED lines, not just the strings the ED
  changes — and any multi-word-phrase grep in a smoke bar must be wrap-tolerant
  (whitespace-normalize via `tr -s '[:space:]' ' '` before grepping, or match a
  single-physical-line sub-phrase). *(Earned, origin ag-os: ED-003's required phrase
  "never build without a GO" wrapped mid-phrase in the portable SKILL.md; smoke bar 4
  went RED and cost a build cycle.)* *(Earned, origin ag-os: ED-009 Bar 4 — the checklist
  walk grep-verified only the two CHANGED strings; the untouched CEREMONY guard literal
  wrapped across lines 57–58 and its single-line grep was unsatisfiable even against the
  cross-vetted package, costing a second build cycle.)*
- [ ] **B6 — Queue-drain loops distinguish per-item permanent failure from transport
  failure.** A drain that "fails safe" by leaving an undeliverable item queued is
  head-of-line blocking: one poison item (oversized body, malformed input, usage error)
  stalls every later item forever. Spec the split explicitly — per-item permanent failure
  → quarantine (dead-letter) + continue + escalate; transport/environment failure → stop
  + retry next drain. *(Earned, origin techne: ED-001's mail relay left failed sends
  queued as its "safe" path; cross-vet found any single bad PM report would silently
  block the team's whole egress plane.)*
...
- [ ] **C9 — Shipped diffs apply cleanly via the executor's exact invocation.** A patch
  in `ED-XX.files/` must be generated with `git diff` (repo-relative `a/`…`b/` headers) OR
  verified at authoring with `patch --dry-run` / `git apply --check` against a clean
  checkout. *(Earned, origin ag-os: ED-023's `service.py.patch` was made with `diff -u`
  against a `/tmp` scratch copy, so its `+++` header carried an absolute path and `patch -p1`
  could not resolve the target — rc1, hunk ignored, tree untouched; recovered via the
  ED-§5-sanctioned exact-string fallback, no build cycle lost, but a git-format patch would
  have applied first try.)*
- [ ] **C8 — Shipped scripts verified runnable exactly as invoked.** If a script re-execs
  itself (`timeout ... "$0"` watchdog or similar), the packaged file needs its exec bit — or
  the re-exec must go through `bash "$0"`. Verify by running the shipped file the way the
  launch prompt will, not a copy in your shell. *(Earned, origin ag-os: ED-006's smoke.sh
  shipped mode 0664; the watchdog re-exec died exit 126 at launch and the executor had to
  chmod +x before bar 0 could run.)*
- [ ] **C10 — §1→§6 coverage mapping total.** Every §1 success-criterion id (`S1`,
  `S2`, ...) appears in at least one §6 bar's `Covers` cell, or in an explicit
  `unverifiable-because-X` ledger line at the §6 bottom. A bar may not claim a
  criterion it does not actually exercise — that is a false tick and cross-vet counts
  it as RED. *(Earned, origin ag-os: adopted 2026-07-12 by user approval (idea.md
  item 1, "Nyquist Validation") on adjacent precedent — ED-021 shipped a VERIFIED bar
  that proved the wrong thing (CSS containment "verified" by hand-parse, actually
  broken), and ED-060's cross-vet found an uncovered exit-code branch that had to be
  hand-proven post-hoc; the clean trigger (a §1 criterion with NO bar at all shipping
  VERIFIED) has not yet occurred and this item exists to keep it that way.)*
- [ ] **C11 — Backgrounded servers: own the REAL PID; assert the port, not the kill.**
  A smoke that starts a server in a backgrounded subshell must `exec` the server inside
  the wrapper (so `$!` is the server, not the subshell) or kill the process group, AND
  assert port-free before each boot and port-freed after each stop. A stale server
  silently answers the next phase's probes with the wrong config — a convincing false-RED
  or false-GREEN. Rehearse the HARNESS's start/stop path, not just the packaged code:
  a rehearsal that kills the server directly never exercises the defect. *(Earned, origin
  ariat-ai-kb: ED-001 Bar 4 false-RED — `(cd ... && uvicorn ...) &` orphaned uvicorn on
  kill; the token-mode server survived into the fail-closed phase and returned 500 where
  the fresh unconfigured server would have returned 503; cost one build cycle. The P5/P6
  authoring rehearsals missed it because they bypassed the harness's stop_server.)*
